...
## histogram test, which are similar? 
provide a counter example to 2003:2004

```{r}

out_1.4$length$og_props %>% 
  tidytable::filter(YEAR %in% 1997:1998) %>% 
...
# now do RSS with boot all + SS
out_2.2$length$rss %>% 
  tidytable::filter(YEAR %in% 2003:2004) %>% 
  tidytable::mutate(Year = as.factor(YEAR)) -> props.sub

p3.5 = ggplot(data = props.sub, aes(x = rss, colour = Year)) +
  geom_histogram(aes(fill = Year, group = Year), alpha = 0.5, position = "identity", binwidth = 200) +
  theme_classic() +
  theme(legend.position = c(0.8, 0.8),
        plot.tag.position = c(0.1, 0.95),
  ...
  geom_point(data = props.sub %>% 
               tidytable::left_join(out_2.2$length$iss %>% 
                                      tidytable::select(YEAR, iss)) %>% 
               tidytable::summarise(x = unique(iss), .by = Year) %>% 
               # tidytable::summarise(x = median(rss), .by = Year) %>% 
               tidytable::mutate(y = c(9,9)), aes(x,y, fill = Year), shape = 25, size = 4, color = "black") +
...
```{r, fig.height=10}
out_1.4$length$og_props %>% 
  tidytable::filter(YEAR %in% 2021:2022) %>% 
  tidytable::mutate(Year = as.factor(YEAR)) -> props.sub
...
p3 = ggplot(data = props.sub, aes(x = rss, colour = Year)) +
  geom_histogram(aes(fill = Year, group = Year), alpha = 0.5, position = "identity", binwidth = 20) +
  theme_classic() +
  theme(legend.position = c(0.8, 0.8),
        plot.tag.position = c(0.1, 0.95),
        plot.tag = element_text(size = 14, face = "bold")) +
...
p3.2 = ggplot(data = props.sub, aes(x = rss, colour = Year)) +
  geom_histogram(aes(fill = Year, group = Year), alpha = 0.5, position = "identity", binwidth = 10) +
  theme_classic() +
  theme(legend.position = c(0.8, 0.8),
        plot.tag.position = c(0.1, 0.95),
        plot.tag = element_text(size = 14, face = "bold")) +
...
ggplot(data = props.sub, aes(x = LENGTH, y = og_FREQ, colour = Year)) +
  ...
  scale_x_continuous(limits = c(22, 108),
                     n.breaks = 9) +
  labs(x = "Length Bin-b (cm)", y = "Proportion (P)", tag = "") +
  theme(legend.position = c(0.8, 0.8),
        plot.tag.position = c(0.1, 0.95),
        plot.tag = element_text(size = 14, face = "bold")) +
  scale_y_continuous(limits = c(0, .032), expand = c(0, 0)) +
...
out_1.4$length$iss$nss[out_1.4$length$iss$YEAR==1997]
```
