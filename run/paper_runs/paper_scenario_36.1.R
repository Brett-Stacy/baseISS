...
### Global
iters_global = 500
samples_global = c(1, 5, 10, 15, 20, 999)
data_global = list("P")
data_names_global = c("P")
...
for(i in 1:length(samples_global)){
  ...
  for(j in 1:length(data_global)){
    ...
    .lfreq_data = lfreq_data %>% tidytable::filter(substr(HAUL_JOIN, 1, 1)=="P")
    ...
    fishery_iss(species_code = "202",
                ...
                new_trip_N = NULL,
                new_haul_N = NULL,
                new_length_N = if(samples!=999){list(type = "value", bound = "minimum", amount = samples, replace = FALSE)}else{NULL},
                boot.trip = TRUE,
                boot.haul = TRUE,
                boot.length = TRUE,
                boot.age = FALSE,
