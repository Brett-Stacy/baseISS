...
rm(list = ls())
run_name = "paper_scenario_40.4"

lfreq_data = readRDS(file = "C:/Users/bstacy/Downloads/y2_nosex_ebs_pcod_Steve_TRIP_STRATA.RDS")
library(tidytable)
library(baseISS)
...
proportion_global = c(0.25, 0.50, 0.75, 1.0)
...
for(i in 1:length(samples_global)){
  ...
  for(j in 1:length(proportion_global)){
    ...
    fishery_iss(species_code = "202",
                ...
                new_trip_N = NULL,
                new_haul_N =list(type = "proportion", amount = .proportion, replace = FALSE),
    ...
    # tictoc::toc()
    out[[paste0("samples_", samples_global)[i]]][[paste0("haul_", proportion_global)[j]]] = .output

  }
}
tictoc::toc()

saveRDS(out, file = paste0("C:/Users/bstacy/baseISS_data/outputs/paper_output/", run_name, "_ISS_", iters_global, "_iters_", Sys.Date(), ".RDS"))
