...
for(i in 1:length(samples_global)){
  ...
  for(j in 1:length(proportion_global$haul)){
    ...
    fishery_iss(species_code = "202",
                ...
                new_trip_N = list(type = "proportion", amount = .proportion_trip, replace = TRUE),
                new_haul_N = list(type = "proportion", amount = .proportion_haul, replace = TRUE),
