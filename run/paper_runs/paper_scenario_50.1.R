...
proportion_global = list(haul = c(.5, .5, .75, .75, 1.0), trip = c(.5, .75, .5, .75, 1.0))
...
for(i in 1:length(samples_global)){
  ...
  for(j in 1:length(proportion_global$haul)){
    ...
    fishery_iss(species_code = "202",
                ...
                new_trip_N = list(type = "proportion", amount = .proportion_trip, replace = FALSE),
                new_haul_N = list(type = "proportion", amount = .proportion_haul, replace = FALSE),
