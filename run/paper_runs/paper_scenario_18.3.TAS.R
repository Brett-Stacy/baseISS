...
for(i in 1:length(proportion_global)){
  ...
  for(j in 1:length(type_global)){
    ...
    fishery_iss(species_code = "202",
                ...
                new_trip_N = if(.type=="trip") {list(type = "proportion", amount = .proportion)} else{NULL},
                new_haul_N = if(.type=="haul") {list(type = "proportion", amount = .proportion)} else{NULL},
                new_length_N = NULL,
                max_length = 150,
                boot.trip = TRUE,
                boot.haul = TRUE,
