# test if expand.by.sampling.strata works for 201

rm(list = ls())
run_name = "generic_201_V2"
iterations = 1
...
# join with initial data frame and rename extrapolated_number to yagmh_snum to get expand_by_sampling_strata to work.
lfreq_data_generic_V4 %>%
  tidytable::left_join(spcomp_data_201_V2) %>%
  tidytable::rename(YAGMH_SNUM = EXTRAPOLATED_NUMBER) -> lfreq_data_generic_V5 # FIXED: joining by distinct haul_join fixed the issue: # major issue: takes forever and too big of an object results
...
fishery_iss(species_code = "201", # called "SPECIES" in non_generic and "SPECIES_KEY" in generic
            ...
            length_based = TRUE,
            iters = 1,
            freq_data = lfreq_data_generic_V5,
            yrs = 1999,
            post_strata = NULL,
