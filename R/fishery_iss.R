...
#' @param new_trip_N list(type = character ("value", or "proportion"), amount = numeric, replace = boolean). If NULL, then the number of trips resampled equals the number actually sampled. If not NULL, then the number of trips is changed by type (acceptable entries are "value", or "proportion") at an amount (acceptable entries are a proportion). E.g., new_trip_N = list(type = "proportion", amount = .50). Only proportion is coded so far as this makes the most sense.
#' @param new_haul_N list(type = character ("value", or "proportion"), amount = numeric, replace = boolean). If NULL, then the number of hauls resampled equals the number actually sampled. If not NULL, then the number of hauls is changed by type (acceptable entries are "value", or "proportion") at an amount (acceptable entries are a proportion). E.g., new_haul_N = list(type = "proportion", amount = .50). Only proportion is coded so far as this makes the most sense.
#' @param new_length_N list(type = character ("value", or "proportion"), bound = NULL or character ("minimum", or "maximum") amount = numeric, replace = boolean). If NULL, then the number of length samples resampled in a haul equals the number actually sampled. If not NULL, then the haul-level number of samples is changed by type (acceptable entries are "value", or "proportion") at an amount (acceptable entries are an integer number or proportion). E.g., new_length_N = list(type = "value", bound = NULL, amount = 20). The amount can be less than or greater than the true N. Note that if it is either less than or greater than, the minimum of the two is chosen for N (this is still under consideration and may change in the future). The true N is taken to be... DOES THIS ONLY MATTER IF WE NEED A THRESHOLD TO DECIDE WHEN TO TAKE THE MAX?? The samples are still drawn with replacement. Warning: cannot do this if post_strata = "SEX"
...
fishery_iss <- function(species_code,
  ...
  # DEV
  # ### Combine post strata by user-defined measurement (weight or numbers) if requested. Keep nested out of this for now, too complicated.
